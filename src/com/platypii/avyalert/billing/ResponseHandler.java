...
public class ResponseHandler {
    ...
    public static void purchaseResponse(final Context context, final PurchaseState purchaseState, final String productId, final String orderId, final long purchaseTime, final String developerPayload) {
        ...
        SharedPreferences.Editor edit = context.getSharedPreferences("AvyAlert", Context.MODE_PRIVATE).edit();
        if(purchaseState == PurchaseState.PURCHASED && productId.equals(Consts.ITEM_PUSH_SUBSCRIPTION)) {
            // Purchased
            edit.putBoolean("isSubscribed", true);
            edit.putLong("purchaseTime", purchaseTime);
            edit.commit();
        } else {
            // Not purchased
